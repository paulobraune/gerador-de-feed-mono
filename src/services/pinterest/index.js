...
async function generateFeedForBusiness(business_id, fileName, options) {
  try {
    logger.info(`Starting Pinterest feed generation for business`, { business_id, fileName });
    const products = await Product.find({ business_id, status: 'active' });

    if (!products || products.length === 0) {
      logger.warn('No active products found for Pinterest feed generation', { business_id });
    }
    ...
    channel.ele('description', {}, `Product feed for Pinterest Catalog`);
    ...
    logger.info('Pinterest feed XML generated successfully', {
    ...
    logger.error('Error generating Pinterest feed', { error: error.message, stack: error.stack });
    throw error;
  }
}
